...
class OpenXRTutorial {
   ...
   public:
    ...
    // Modified from
    // https://github.com/KhronosGroup/OpenXR-SDK-Source/blob/d6b6d7a10bdcf8d4fe806b4f415fde3dd5726878/src/tests/hello_xr/main.cpp#L133C1-L189C2
    ...
    static void AndroidAppHandleCmd(struct android_app *app, int32_t cmd) {
        ...
        switch (cmd) {
            // There is no APP_CMD_CREATE. The ANativeActivity creates the application
            // thread from onCreate(). The application thread then calls android_main().
            case APP_CMD_START: {
                break;
            }
            case APP_CMD_RESUME: {
   ...
   private:
    void CreateInstance() {
        ...
        // runtime. If an extension is found add it to Active Instance Extensions.
    ...
    void DestroyInstance() { OPENXR_CHECK(xrDestroyInstance(m_xrInstance), "Failed to destroy Instance."); }
    ...
    void PollSystemEvents() {
        // Checks whether Android has requested that application should by
        // destroyed.
    ...
    void GetViewConfigurationViews() {
        ...
        OPENXR_CHECK(
            xrEnumerateViewConfigurations(m_xrInstance, m_systemID, viewConfigurationCount, &viewConfigurationCount, m_viewConfigurations.data()),
            "Failed to enumerate View Configurations.");
        ...
        OPENXR_CHECK(xrEnumerateViewConfigurationViews(m_xrInstance, m_systemID, m_viewConfiguration, viewConfigurationViewCount,
    ...
    void CreateSwapchains() {
        ...
        for (size_t i = 0; i < m_viewConfigurationViews.size(); i++) {
            ...
            swapchainCI.sampleCount =
                m_viewConfigurationViews[i].recommendedSwapchainSampleCount;  // Use the recommended values from the XrViewConfigurationView.
    ...
    void GetEnvironmentBlendModes() {
        ...
        OPENXR_CHECK(xrEnumerateEnvironmentBlendModes(m_xrInstance, m_systemID, m_viewConfiguration, environmentBlendModeCount,
    ...
    void CreateReferenceSpace() {
        // Fill out an XrReferenceSpaceCreateInfo structure and create a reference XrSpace, specifying a Local space with an identity pose as the
        // origin.
    ...
    bool RenderLayer(RenderLayerInfo &renderLayerInfo) {
        ...
        for (uint32_t i = 0; i < viewCount; i++) {
            ...
            OPENXR_CHECK(xrAcquireSwapchainImage(colorSwapchainInfo.swapchain, &acquireInfo, &colorImageIndex),
...
void OpenXRTutorial_Main(GraphicsAPI_Type apiType) {
    DebugOutput debugOutput;  // This redirects std::cerr and std::cout to the
                              // IDE's output or Android Studio's logcat.
...
void android_main(struct android_app *app) {
    ...
    // Load xrInitializeLoaderKHR() function pointer. On Android, the loader must
    // be initialized with variables from android_app *. Without this, there's is
    // no loader and thus our function calls to OpenXR would fail.
    ...
    // Fill out an XrLoaderInitInfoAndroidKHR structure and initialize the loader
    // for Android.
